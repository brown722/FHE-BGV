...
/*Precomputations for following FFTs*/
	...
	// 		invPrimes[i], primes[i], nx, powers_gpu[i], r_smallSample[i], gpuOffsetFlags, offset[i], streamSmall[i], streamGauss[i], planSmall[i], invPlanSmall[i], 0);
	// }
	devBluestn2(mm, smallBuffer, cuSmallBuffer,  cuRaSmall, Rb_gpu, resultSmall, cuFFToutSmall, 
		invPrimes, primes, nx, powers_gpu, r_smallSample,  gpuOffsetFlags, offset, streamSmall, streamGauss, planSmall, invPlanSmall);
	...
	for (int i = 0; i < numPrimes; i ++)
	{
		deviceBlustn2(mm, gaussBuffer[i], cuGaussBuffer[i],  cuRaGauss[i], Rb_gpu[i], resultGauss[i], cuFFToutGauss[i], 
			invPrimes[i], primes[i], nx, powers_gpu[i], e_gaussianSample[i], gpuOffsetFlags, offset[i], streamGauss[i], streamSmall[i],planGauss[i],  invPlanGauss[i], 1);
	}
	...
	for (int i = 0; i < numPrimes; i ++)
	{
		deviceBlustn2(mm, ptxtBuffer[i], cuPtxtBuffer[i],  cuRaPtxt[i], Rb_gpu[i], resultPtxt[i], cuFFToutPtxt[i], 
			invPrimes[i], primes[i], nx, powers_gpu[i], ptxt[i], gpuOffsetFlags, offset[i], streamSmall[i], streamGauss[i], planPtxt[i],  invPlanPtxt[i], 0);
	}
