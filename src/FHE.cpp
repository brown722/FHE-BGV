			...
			bool highNoise) const
{
  ...
  ctxt = pubEncrKey;  // already an encryption of zero, just not a random one// assigning parts primeSet  ptxtSpace noiseVar
  ...
  for (size_t i=0; i<ctxt.parts.size(); i++) {  // add noise to all the parts //for all parts
    ctxt.parts[i] *= r;
