...
class PAlgebra {
  ...
  // ords[i] is the order of gens[i] in quotient group kept
 ...
 public:
  vector<long> dLogT; //was private
  vector<long> gens;//was private
  vector<long> ords;//was private
  unsigned long ordP; // the order of p in (Z/mZ)^*//was private
  ...
  void getZmsIndex(vector<long> &zmxIdx) {zmxIdx = this->zmsIdx;}
...
public:
	...
	PA_INJECT(type)
	REContext contextForG; //was private;
	vector<RX> maps;//was private
	vector<REX> rmaps;//was private;
...
public:
  PA_INJECT(type)
vec_RX crtCoeffs;//was private
vec_RX factors;//was private;
vector<RX> crtTable;//was private
shared_ptr< TNode<RX> > crtTree;//was private
private:
...
private:
  ...
  void mapToF1(RX& w, const RX& G) const { mapToFt(w,G,1); }//maps[0], mappingData.G
