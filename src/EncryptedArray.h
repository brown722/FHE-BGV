...
private:
  ...
  void genericEncode(ZZX& ptxt, const T& array) const
  {
    ...
    vector< RX > array1;
    convert(array1, array);//(vector< RX >, Vector<long>)
    ///cout<<"zz_pX vector is "<<endl<<array1<<endl; //my_flag
    encode(ptxt, array1);
  }
                      ...
                      const T& array) const
  {
    ...
    pKey.Encrypt(ctxt, pp, tab.getPPowR()); // encrypt the plaintext polynomial// p^r: the plaintext space
    //cout<<pp;//my_flag
  }
